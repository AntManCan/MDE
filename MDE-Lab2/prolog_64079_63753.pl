...
addcond(PP, arc(X,Y,L), TP) :-
    not(member(arc(Y,X,_),PP);member(arc(Y,X,_),PP)),
...
addcond_tri(PP, arc(X:E1,Y:E2), TP) :-
    not(member(arc(Y:_,X:_),PP);member(arc(X:_,Y:_),PP)),
...
instexec(1):-write('Installation Name?'),nl,
             ...
             write('Power: '),read(PowerM),addInst(InstName, 'monofasico',PowerM));
             (   (PowerType == 3) ->
             (   write('Power: 13.80 or 20.7'),nl,
             write('Power: '),read(Power),addInst(InstName, 'trifasico',Power)))),
             write('Installation added successfully!'),nl.
instexec(1):-w_error(1).

instexec(2):-write('Installation Name?'),nl,
   read(InstName),
   removeInst(InstName),
   write('Installation removed successfully!'),nl.
...
instexec(3):-write('Installation Name?'),nl,
   ...
   write('Installation modified successfully!'),nl.
...
deviceexec(3):-write('Device Ref?'),nl,
   ...
   read(NewDName),
   dispositivo(DRef,_,Dnum),
   alteraDispositivo(DRef,NewDName, Dnum),
   write('Device modified successfully!'),nl.
...
powermenu(POp):-  write('====[ Power Menu ]===='),nl,
                    write('    1. Edit a Device Power.'),nl,
                    write('    2. Go back.'),nl,
...
powervalid(POp):- POp >= 1, POp=<2.

powerexecute(2).
powerexecute(POp):- powerexec(POp), nl,mainexec(4).

powerexec(1):-write('Device Ref?'),nl,
   read(DRef),
   write('New Device Power?'),nl,
   read(DPower),
   dispositivo(DRef,Dname,_),
   alteraDispositivo(DRef,Dname, DPower),
   write('Power modified successfully!'),nl.
powerexec(1):-w_error(1).
...
listexec(3):-write('Installation Name?'),nl,
   read(InstName),
   inst_pot_total(InstName,C),
   write('List operation successful!'),nl,
   write('Installation Total Power Consumption: '),write(C),nl.
listexec(3):-w_error(1).

listexec(4):-write('Device Ref?'),nl,
   read(DRef),
   listInstDType(DRef, LI) ,
   write('List operation successful!'),nl,
   write('Installation List: '),write(LI),nl.
listexec(4):-w_error(1).

listexec(5):-instMaiorConsumo(I),
   write('List operation successful!'),nl,
   write('Installation List: '),write(I),nl.
listexec(5):-w_error(1).
...
miscexecute(4).
miscexecute(IOp):- miscexec(IOp), nl, mainexec(6).
